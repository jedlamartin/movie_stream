...
int main(int argc, char* argv[]) {
    ...
    if(setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) !=
       0) {
        fprintf(
            stderr, "setsockopt(SO_REUSEADDR) failed: %s\n", strerror(errno));
        exit(1);
    }
    ...
    while(1) {
        ...
        // Accept
        if((*client_fd =
                accept(socket_fd, (struct sockaddr*) &addr, &client_len)) < 0) {
...
void printusage(char* progname, int fd) {
    ...
    dprintf(
        fd, "  -p port   Specify the port to listen on (default: %d)\n", PORT);
    dprintf(fd,
            "  -c max_connections   Specify the maximum simultaneous client "
