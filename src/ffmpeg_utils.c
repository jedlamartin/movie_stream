#define _DEFAULT_SOURCE
#include "ffmpeg_utils.h"

#include <ctype.h>
#include <libavcodec/avcodec.h>
#include <libavformat/avformat.h>
#include <libavutil/dict.h>
#include <libavutil/log.h>
#include <limits.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>

void sanitize_name(char* dst, const char* src, size_t max_len) {
    size_t j = 0;
    for(size_t i = 0; src[i] != '\0' && j < max_len - 1; i++) {
        unsigned char c = src[i];
        if(isalnum(c)) dst[j++] = c;
        else
            dst[j++] = '_';
    }
    dst[j] = '\0';
    while(j > 0 && dst[j - 1] == '_') dst[--j] = '\0';
    if(j == 0) strcpy(dst, "und");
}

TrackInfo get_track_counts(const char* filename) {
    TrackInfo info;
    memset(&info, 0, sizeof(TrackInfo));
    AVFormatContext* fmt_ctx = NULL;
    av_log_set_level(AV_LOG_QUIET);

    if(avformat_open_input(&fmt_ctx, filename, NULL, NULL) < 0) {
        info.error = 1;
        return info;
    }
    if(avformat_find_stream_info(fmt_ctx, NULL) < 0) {
        avformat_close_input(&fmt_ctx);
        info.error = 1;
        return info;
    }

    int a_idx = 0, s_idx = 0, global_s_idx = 0;
    for(unsigned int i = 0; i < fmt_ctx->nb_streams; i++) {
        AVStream* st = fmt_ctx->streams[i];
        AVDictionaryEntry* lang =
            av_dict_get(st->metadata, "language", NULL, 0);
        AVDictionaryEntry* title = av_dict_get(st->metadata, "title", NULL, 0);

        if(st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
           st->disposition != AV_DISPOSITION_ATTACHED_PIC) {
            info.video_count++;
        } else if(st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            if(a_idx < MAX_TRACKS) {
                if(lang) {
                    strncpy(info.audio[a_idx].lang, lang->value, 3);
                    info.audio[a_idx].lang[3] = '\0';
                } else
                    strcpy(info.audio[a_idx].lang, "und");

                if(title) strcpy(info.audio[a_idx].title, title->value);
                else
                    strcpy(info.audio[a_idx].title, info.audio[a_idx].lang);
                sanitize_name(
                    info.audio[a_idx].title, info.audio[a_idx].title, 63);

                int dup_count = 1;
                for(int k = 0; k < a_idx; k++) {
                    if(strncmp(info.audio[k].title,
                               info.audio[a_idx].title,
                               63) == 0)
                        dup_count++;
                }
                if(dup_count > 1) {
                    char suffix[16];
                    snprintf(suffix, sizeof(suffix), "_%d", dup_count);
                    strcat(info.audio[a_idx].title, suffix);
                }
                a_idx++;
            }
            info.audio_count++;
        } else if(st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE) {
            int codec = st->codecpar->codec_id;
            // Detect if FFmpeg can safely extract it to WebVTT
            int can_extract =
                (codec == AV_CODEC_ID_SUBRIP || codec == AV_CODEC_ID_ASS ||
                 codec == AV_CODEC_ID_SSA);

            if(s_idx < MAX_TRACKS && can_extract) {
                info.subs[s_idx].index = global_s_idx;
                if(lang) {
                    strncpy(info.subs[s_idx].lang, lang->value, 3);
                    info.subs[s_idx].lang[3] = '\0';
                } else
                    strcpy(info.subs[s_idx].lang, "und");

                if(title) strcpy(info.subs[s_idx].title, title->value);
                else
                    strcpy(info.subs[s_idx].title, info.subs[s_idx].lang);
                sanitize_name(
                    info.subs[s_idx].title, info.subs[s_idx].title, 63);

                int dup_count = 1;
                for(int k = 0; k < s_idx; k++) {
                    if(strncmp(
                           info.subs[k].title, info.subs[s_idx].title, 63) == 0)
                        dup_count++;
                }
                if(dup_count > 1) {
                    char suffix[16];
                    snprintf(suffix, sizeof(suffix), "_%d", dup_count);
                    strcat(info.subs[s_idx].title, suffix);
                }
                s_idx++;
            }
            global_s_idx++;
        }
    }
    avformat_close_input(&fmt_ctx);
    info.subtitle_count = s_idx;
    return info;
}

int generate_hls_with_tracks(const char* mkv_path, const char* hls_dir) {
    TrackInfo info = get_track_counts(mkv_path);
    if(info.error || info.video_count == 0) return -1;

    char abs_mkv_path[PATH_MAX];
    if(!realpath(mkv_path, abs_mkv_path)) strcpy(abs_mkv_path, mkv_path);

    // 1. SIDECAR SUBTITLE EXTRACTION
    for(int i = 0; i < info.subtitle_count; i++) {
        char sub_cmd[PATH_MAX + 256];
        snprintf(sub_cmd,
                 sizeof(sub_cmd),
                 "ffmpeg -y -i \"%s\" -map 0:s:%d -f webvtt \"%s/sub_%d.vtt\" "
                 "> /dev/null 2>&1 &",
                 abs_mkv_path,
                 info.subs[i].index,
                 hls_dir,
                 i);
        system(sub_cmd);
    }

    // 2. BULLETPROOF VIDEO & AUDIO MUXING
    char map_args[2048] = "-map 0:v:0";
    char var_stream_map[4096] = "v:0,agroup:audio ";

    for(int i = 0; i < info.audio_count; i++) {
        char buf[128];
        // FIX 1: Added "-ac:a:%d 2" to force downmixing 5.1/7.1 audio to
        // 2-channel stereo
        snprintf(buf, sizeof(buf), " -map 0:a:%d -ac:a:%d 2", i, i);
        strcat(map_args, buf);

        char vsm_buf[256];
        snprintf(vsm_buf,
                 sizeof(vsm_buf),
                 "a:%d,agroup:audio,language:%s,name:%s,default:%s ",
                 i,
                 info.audio[i].lang,
                 info.audio[i].title,
                 (i == 0 ? "yes" : "no"));
        strcat(var_stream_map, vsm_buf);
    }

    char cmd[16384];
    snprintf(cmd,
             sizeof(cmd),
             "ffmpeg -i \"%s\" %s "
             "-c:v copy -tag:v hvc1 -c:a aac -b:a 128k "
             // FIX: Added "-hls_segment_type fmp4" here:
             "-f hls -hls_segment_type fmp4 -hls_time 10 -hls_list_size 0 "
             "-hls_flags independent_segments "
             "-master_pl_name master.m3u8 "
             "-var_stream_map \"%s\" "
             "\"%s/stream_%%v.m3u8\" > \"%s/ffmpeg.log\" 2>&1 & "
             "PID=$!; echo $PID > \"%s/.pid\"; wait $PID",
             abs_mkv_path,
             map_args,
             var_stream_map,
             hls_dir,
             hls_dir,
             hls_dir);

    return system(cmd);
}